    ...
    <div class="dictionary-choice-viewport no-scrollbar">
      <div class="dictionary-choice-options" :class="{ 'is-dense': options.length > 8 }">
        <button
          ...
          @click="!disabled && !option.disabled && emit('select', option.key)"
        >
...
button.unavailable:disabled:hover { border-color: color-mix(in srgb, var(--border-color) 72%, transparent); background: color-mix(in srgb, var(--bg-card) 62%, transparent); opacity: .52; }
