...
export interface Bunsetsu { morphemes: Morpheme[]; surface: string; head_word: HeadWord; grammar_tags: GrammarTag[]; word_formations: WordFormationAnnotation[]; char_range: [number, number]; }
...
export interface ExpressionRule { id: number; label: string; description: string; origin: string; expression_type: ExpressionType; priority: number; boundary_effect: ExpressionBoundaryEffect; parts: ExpressionPatternPart[]; created_at: string; gap_after?: number; gap_bunsetsu?: [number, number]; }
export interface ExpressionAnnotation { match_id: string; rule_id: number; label: string; description: string; origin: string; expression_type: ExpressionType; priority: number; boundary_effect: ExpressionBoundaryEffect; confidence: number; position: "start" | "middle" | "end" | "single"; token_range: [number, number]; char_range: [number, number]; surface: string; }
