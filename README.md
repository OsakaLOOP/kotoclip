...
主界面可直接粘贴 Markdown 文本，也可点击“导入 EPUB”选择本地 `.epub` 文件。EPUB 解包、OPF/spine 解析、XHTML 转换、ruby 注音规范化和 Markdown 清洗均由 `kotoclip-core` 内的 Rust 导入器完成；生成结果先进入可编辑输入区，确认后再沿用现有 `open_document` 文档会话与渐进分析管线。
