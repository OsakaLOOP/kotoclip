...
主界面可直接粘贴 Markdown 文本，也可点击“导入 EPUB”选择本地 `.epub` 文件。EPUB 解包、OPF/spine 解析、XHTML 转换、ruby 注音规范化和 Markdown 清洗均由 `kotoclip-core` 内的 Rust 导入器完成。Markdown frontmatter 会提取为内部标题、作者、日期和语言元数据，并从输入正文中剥离；阅读器顶部显示标题与作者，只有正文进入 `open_document` 文档会话与渐进分析管线。
