...
请求读音明确冲突的 occurrence 在此阶段排除。每个原始表记在 variant 中聚合当前读音
条件下的读音、证据、得分和词典来源。
...
SQLite schema v4 的 `entries` 表没有结构化 `entry_kind`。运行时按每个候选 variant 和
词典精确加载并适配记录，随后使用读音条件与实质正文条件确认单元格。仅有索引、
navigation/redirect 或冲突读音均不会形成可用单元格；零可用单元格的表记种子在生成
`forms[]` 前删除。
...
`dictionaries[]` 读取可用性。`dictionaryMatrix.ts` 负责解析合法组合：选择表记时保留
兼容词典或按词典顺序回退，选择词典时保留兼容表记或按表记顺序回退。当前组合始终
对应可用单元格，行列集合不随选择变化。
