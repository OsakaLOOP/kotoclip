...
- GrammarPopover 使用单卡片核心呈现：同形单语素不重复标题、本句、捕获和构成；只有发生表面变形、多语素组合或非平凡活用时才显示“本句形态／辞书形回指／组成／活用”。词典 TooltipPanel 同步消费词汇所有权的活用链，显示合并后的本句词形、语言学辞书形和活用步骤；词典实际查询词仍保持原有策略。
...
2. 每条链明确记录 `lexical` 或 `functional` 所有权，以及 `surface_form`、`dictionary_form`、`lookup_form`、语素范围和字符范围。
...
5. 紧凑 IPC 正式传输 morphology；悬浮词汇链任一组成语素时，词典气泡显示合并词形及活用卡片。
...
变体信息只在确有信息时显示：实际表面不同于 canonical form、capture 的 surface 不同于 base form、多语素构式或存在非平凡 morphology operator。显示形式优先使用“表面 ← 辞书形”、带角色的组成片段和短活用 badge。
...
1. 鼠标位于词汇链任一原始语素时，统一解析为合并后的词形目标；
2. 实际词典查询仍使用既有 `head_word.base_form`，不因本次改造强制改查 `理解する／静かだ`；
3. TooltipPanel 在词典正文前显示“本句形态”“表面形 ← 语言学辞书形”和有序活用步骤；
4. 功能用言继续由 GrammarPopover 解释，其变形词尾包含在同一蓝色命中范围内。
