...
状态：已实现并完成词汇活用边界纠偏（2026-07-16）

2026-09-11 平行重写进度：UniDic 统一管线已提供 `kotoclip.morphology-artifact.v1` 形态链和功能语素 `candidate` occurrence；编译语法目录与讲解库已通过 Tauri 只读查询命令接入。语法构式识别、sense resolution 和正文讲解投影仍按本文 G3～G5 顺序推进。
...
- `pipeline/morphology` 直接在原始 IPADIC 语素序列上输出独立 `MorphologyArtifact`、operator chain、连接形及活用 occurrence；普通词与功能用言共享同一活用层，文节原子连接、词汇展示范围和语法识别消费同一结果。
...
- 当前编译产物包含 105 个 concept、154 个 sense、112 个 realization/rule、105 份讲解和 0 个 redirect；目录审计中的悬空引用、不可发布规则、缺失来源和缺失搜索项均为 0。
...
- GrammarPopover 使用单卡片核心呈现：同形单语素不重复标题、本句、捕获和构成；只有发生表面变形、多语素组合或非平凡活用时才显示必要的形态、组成和活用信息。词典 TooltipPanel 通过统一词形 view model 消费被悬浮的具体活用链：表头左侧显示规范原型、读音和词性，右侧显示实际形及带短解说的活用步骤；词典实际查询仍使用链自身的既有查询形。
- 讲解资产显式记录生成来源、作者、日期、版本和默认核验等级。气泡以来源图标、合并后的“作者·日期·版本”和核验 SVG badge 呈现；文法库允许在本机覆盖未核验、AI 批量核验、成员权威核验三档状态。
...
必要验证结果：原 7 个代表句继续覆盖识别、文本重建和讲解解析；新增固定用例覆盖 `見えなかった`、`記録された`、`分類し`、`読んでくださった`、`静かな／応用的な` 与反例 `行くな`。本次执行了 Morphology 单元测试、真实 IPADIC 管线验收、前端生产构建和目录重建校验。`incremental-consistency` 仅保留为未来管线迁移或失效策略重构的专项诊断，不属于当前模块日常验收。

### 0.1 2026-07-16 词汇活用边界审计

本次审计确认，旧实现虽然生成了 `MorphologyArtifact`，但它位于文节和词典候选之后，只作为 GrammarOccurrence／GrammarTag 的解释投影存在。由此产生四个根本偏离：

- `head_word` 无法消费活用链，前端只能把单个词干染成黄色，否定、过去、使役、受身及サ变词尾留为灰色或蓝色；
- `名詞・サ変接続＋する` 只保留名词词头，无法把 `理解する／分類し` 作为同一词汇形态展示；
- `形容動詞語幹＋だ・体言接続` 被当作独立功能语素，`静かな／応用的な` 中的 `な` 错误脱离词汇核心；
- 词汇谓词和补助用言没有活用链所有权，普通动词的活用与 `くださった` 等功能用言活用混入同一蓝色投影。

纠偏后的稳定契约为：

1. `analyze_morphemes` 是不依赖文节和语法目录的公共入口，先恢复连续活用链；原始 Morpheme 和字符坐标不变。
2. 每条链明确记录 `lexical` 或 `functional` 所有权，以及 `surface_form`、`lemma_form`、`dictionary_form`、`lookup_form`、语素范围和字符范围。`lemma_form` 是默认 UI 原型，`dictionary_form` 是内部语言学还原，`lookup_form` 是实际词典查询形。
3. 文节分析把活用链内部边界视为原子连接；词汇链扩展 `head_word.surface`，但不改变现有 `head_word.base_form` 和词典查询策略。
4. 词汇链的通用活用不再投影为蓝色 GrammarTag；功能用言链的变形范围扩入所属功能语素或构式，例如 `くださっ＋た` 统一进入 `〜てくださる` 的蓝色范围和讲解。
5. 紧凑 IPC 正式传输 morphology；正文着色、悬浮目标和词典表头统一消费链的精确范围与身份，不再从 `head_word.surface/base_form` 反向猜测。
6. ナ形容词的默认显示原型保留连体形 `〜な`，例如 `静かな`；内部 `dictionary_form` 仍可保留 `静かだ`，但不得覆盖用户标题。
...
- 不让功能语素／语法构式层反向改写 Morpheme、WordFormation、DictionaryLexicalUnit 或 Bunsetsu 的规范边界；通用活用层可以把已确认的连续活用链声明为文节原子连接，并扩展词汇展示范围。
...
建议执行顺序：
    ...
    TextPreparation
      → Morpheme
      → MorphologyAnalysis / MorphologyAtomicRanges
      → WordFormation
      → DictionaryLexical
      → BunsetsuAnalysis
...
- MorphologyAnalysis 不重写 IPADIC 输出，但其连续范围参与文节原子连接和词汇 `head_word.surface` 投影。
...
`dictionary_form` 是语言学上的完整辞书形，例如 `分類する`、`静かだ`；`lookup_form` 保留项目既有词典检索策略，例如分别查询 `分類`、`静か`。两者不得混用。
...
展示所有权按 occurrence 决定：前项词汇链首先把连接形纳入黄色核心；若同一 `て／で` 还被正式功能语素或构式命中，蓝色解释入口可以覆盖该精确范围，但不得把前项词干重新拆开。
...
- `名詞・サ変接続＋する`：`理解する／分類し／記録された` 的 `する` 及其后续通用活用属于词汇链，词典查询仍使用既有名词词头。
- `形容動詞語幹＋だ`：`静かな／応用的な` 的 `な` 是 `だ` 的体言接续形，属于词汇链；句末终助词 `行くな` 不满足该身份条件，继续作为独立语法对象。
...
### 9.3 GrammarExplanationDocument
    ...
    ├─ authoring_status
    ├─ content_version
    ├─ provenance { origin, author, date, version }
    ├─ review_status
    └─ body_blocks[]
...
| lexical 活用链 | 与词汇核心一致的黄色／画像颜色；词典气泡显示活用卡片 |
| functional 活用链 | 并入所属功能语素或构式的蓝色范围，不单独生成聚合 badge |
...
- 多语素构式生成一个短 badge；
- 词汇谓词链不生成语法 badge，在词典气泡中聚合显示；功能用言链并入所属语法对象，不重复生成活用 badge；
- popover 内再展开组成特征、候选义项和证据；
- 被更具体构式 refine 的通用构式不重复展示 badge。

### 12.4 分层显示

GrammarPopover 保留为唯一语法浮层组件，并使用一张完整但克制的卡片，不再设置与内容量无关的展开按钮。默认顺序为：知识类型与标题、核心功能、多义候选、必要的本句形态、有效接续和补充说明、词典入口、来源与核验 badge。内容过长时只在面板内部滚动，不扩大正文布局。

呈现层必须先消重：标题相同的 actual form 不重复；单一 capture 不再另列“本句捕获”；`occurrence_binding` 与核心摘要相同则隐藏；通用免责 warning、原始 rule ID、IPADIC evidence、目录内部状态和裸 source ref 不进入默认气泡。

变体信息只在确有信息时显示：实际表面不同于 canonical form、capture 的 surface 不同于 base form、多语素构式或存在非平凡 morphology operator。GrammarPopover 可继续使用带角色的组成片段；TooltipPanel 不重复“表面 ← 原型”，而是在同一表头右侧显示实际形，并为每个活用步骤同时显示短标签和简短规则说明。
...
普通词汇活用使用现有词典悬浮会话，不新增独立浮层：

1. 鼠标位于词形链任一原始语素时，以该链的 `source_ranges` 精确解析悬浮目标；同一文节的多条链不得互相覆盖。
2. 标题使用 `lemma_form`：サ变显示 `理解する／説明する`，ナ形容词显示 `静かな／応用的な`，普通用言显示自身辞书形。
3. 实际词典查询使用被悬浮链的 `lookup_form`；主词链仍保持 `理解／説明／静か` 等旧查询策略，`やすく` 等第二词形链则查询自己的 `やすい`，不回退到文节词头。
4. TooltipPanel 表头左侧显示原型、规范读音和词性；右侧仅在有表面变化或活用步骤时显示实际形、活用标签及其简短解说。
5. lexical chain 范围进入黄色词汇样式；functional chain 与 accepted 语法范围进入蓝色样式；辅助灰色只表示尚未归类的语素。
6. 功能用言仍优先由 GrammarPopover 解释，其变形词尾包含在同一蓝色命中范围内；没有更具体 occurrence 时仍保留功能链自己的词形查询身份。
...
- 词汇活用链完整进入黄色核心，サ变与形容动词连体 `な` 不再留下灰色缝隙；
- 语法弹层和现有词典内部目标可以互相到达；
