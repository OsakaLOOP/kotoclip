...
| P4 语言分析 | 需修订 | 活用链、来源构词候选、词典整体绑定、阅读单位和解释投影已经生成；现有构词、语法和表达结果不构成完整验收证据 | 按 P4 审计重建结构化语法与表达识别；P5 完成查询矩阵和整体／内部交互 |
...
P0 至 P3 已完成。P4 的来源证据、活用链和部分词汇对象可继续使用，其余修订范围见 [P4 十段输出逐层审计](p4_layer_audit.md)。P5 词典解释交互可基于现有来源与词典引擎继续分析；会话证据保存在 `data/validation/behavior/sessions.json`，桌面短例证据保存在 `data/validation/behavior/session-desktop.json`。
