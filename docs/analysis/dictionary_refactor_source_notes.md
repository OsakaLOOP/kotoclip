...
1. 查询读音 `けはい` 时，Crown、小学馆和大辞林 `けはい` 都是当前 occurrence 的直接候选；大辞林 `きはい` 是词典中的另一真实 occurrence，只在查询 `きはい` 或未指定读音时进入当前单元格。
...
1. 当前读音 `にんげん` 下 Crown、小学馆和大辞林都是直接候选；大辞林同表记另有 `じんかん`、`ひとま` 两个真实 occurrence，分别由对应读音查询进入单元格。
...
1. 请求 `前/まえ` 时三本词典的 `まえ` occurrence 为直接候选；`ぜん`、`さき` 及大辞林汉字条均为词典中的真实记录，在当前请求中由读音条件排除。
...
5. Crown `前/ぜん` 原文只有英文 `ex-` 和三个前接例句。省略英文后不能留下空 lexical sense；根据显式连字符结构标记为 `prefix`，表头显示接头成分，例句保留。它在 `ぜん` 请求下进入候选，在 `まえ` 请求下由读音条件排除。
6. 大辞林 `先/さき【先・前】` 是同表记异读 lexical occurrence；汉字条 `前/ぜん` 是 `kanji` entry。`まえ` 查询会在读音条件阶段排除二者。
