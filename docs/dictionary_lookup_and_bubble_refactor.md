...
`行く` 在全局表记列表中只有一行。Crown、大辞林和小学馆各自保存的
`いく/ゆく` occurrence 留在三个单元格内。
...
同一词典可包含不同读音、词性、记录类型或同形同读记录。它们只在当前单元格中选择。
...
同一已准入表记的其他真实读音继续保留。例如 `強い/コワイ` 的活动单元格同时保存
`こわい/つよい`。读音不兼容且没有 query/observed 精确证据的 `縄/名和/那波`
不会进入 `なれる` 矩阵。
...
每个表记组返回全部已启用词典列。可用性由组内 variants 的精确索引与已解析记录共同确定，
与单个 variant 的来源词典分开保存。
已经确认为 navigation/redirect 且没有实质正文的单元格标记为不可用。
...
随后按 occurrence ID 去重。非活动表记只保留矩阵元数据。
...
表记超过 8 项时使用集中菜单。词典按钮始终显示；不可用按钮暗显并禁止触发路由。
活动词典在表记切换后可以保持为不可用状态，正文区显示单元格空态。
