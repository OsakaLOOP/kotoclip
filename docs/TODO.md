...
核对日期：2026-09-22。当前核对基线为 `522a801`，本轮只调整文档和计划。P4 现状与独立待核验项见 [完成范围核对](p4_handoff.md)，下一模块协议见 [词典与解释](dictionary.md)。
...
| GiNZA／KWJA | 常驻服务、配置、并行调用、缓存和实际桌面入口 | GiNZA 支持词汇主流程，KWJA 及依存等结构能力按实验消费 |
...
P0–P3 已完成基础接入；P4 提供 token、活用链和构词候选。P4 自身核验及修复独立处理。长期顺序为 P5A 查询目标、P5B 词典聚合与决定、P5C 短文本交互、P5D 语法与表达，再接入 P6 阅读器和 P7 持久化。P5D 可在 P5A 对象稳定后单独推进；P8–P10 保留助手、审计和完整交付范围。
...
- [ ] 定义整体词、内部子词、词元、功能成分和有词汇依据的 token 内部目标，建立包含、等价及交叉候选关系，保存 P4 来源引用。
- [ ] 统一目标的会话、单元、产物版本及字符锚点，完成查询和重算后失效校验；以 A28–A33 建立人工期望及查询快照。

### P5B：词典验证、聚合与决定

- [ ] 基于 P4 候选和受边界限制的索引查询发现合理范围，完成词典验证、备选保留、默认目标及无整体记录时的内部查询。
- [ ] 合并目标全部合法词形的矩阵，按上下文基本形选择默认项；保留表记／读音配对、词性和每个词典记录身份，修复 `読みませんでした`、`てる` 等已记录的选择问题。
...
本节承接原 P4 尚未完成的语言能力，与词典及查询模块分别实施。

- [ ] 消费活用 occurrence 和必要结构证据，完成构式识别、语境义项及正文解释，接入文法库和核验状态。
- [ ] 完成连续／非连续表达识别、功能所有权、精确范围及硬边界；恢复表达规则编辑、预演、保存和重算刷新。旧通用扫描的替换与遗留入口清理在本节独立完成。
- [ ] 建立出现级结构与解释修正、复核及撤销协议，更新当前会话投影；持久化和重启恢复归 P7。完成 A09、A10。
...
- [ ] 持久化 P5D 的实例修正和撤销，完成重启恢复及迁移；检查来源或文本更新后的有效性。
