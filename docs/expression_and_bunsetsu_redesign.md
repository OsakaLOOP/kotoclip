...
“整体查不到词典”不是拆成带空格胶囊的充分条件。只要构词结构成立，显示仍应连续；悬浮可以下钻内部语素释义。
...
`kotoclip-cli schema-audit [--json PATH]` 是进入阶段 E 前的固定机器入口。加入词典词汇层后当前为 4 层、46 条启用规则；各层严格校验并输出去重能力：

| 层 | schema | 规则 | 已落实的关键能力 |
| --- | ---: | ---: | --- |
| WordFormation | 2 | 5 | 四级词性、活用类型精确/前缀、活用形、有界重复、命名捕获、类型化词头 |
| DictionaryLexical | 1 | 4 | 精确表记绑定、语素窗口、结构分流、区间动态规划、构词交叉保护、稳定词条键 |
...
词典查询不再沿用单次 `head_word` 查询。整体词汇、生产型构词成分、未来数量读法资源的查询协议、可选双面板交互，以及与文节末尾语法 badge 的职责边界，见 [`explanation_targets_and_dictionary_ui.md`](explanation_targets_and_dictionary_ui.md)。该模块只消费 B/C/D 的正式注解，不反向修改分析边界。

此前后置词典边界只能处理单一既有文节，系统性漏掉 `マジックミラー／血飛沫／正当防衛／過剰防衛` 等被提前切开的精确词典整体。补充方案见 [`dictionary_lexical_units.md`](dictionary_lexical_units.md)：在 Bunsetsu 前生成 `DictionaryLexicalCandidate`，与 WordFormation 共享跨度决策但保持不同注解；无助词词汇整体从 D 阶段 pending 分流，含助词和句法结构继续留在表达层。

**词典词汇整体与双词典浮层已完成（2026-07-13）**：前三话 accepted 词汇整体为 318／325／309，Bunsetsu 数更新为 10375／9035／7758；D 层 accepted 仍为 39／11／13，pending 降为 88／76／67。完整接口、冲突策略、基线和 UI 交接均已记录在上述两份专项文档，下次会话无需重新审计。

阶段 E 不在旧表达编辑器上继续叠加状态。直接重建一套覆盖构词、文节和表达规则的统一工作台；旧 UI 仅作为交互素材参考。

**第一轮重写已完成（2026-07-13）**：

- `RuleWorkbench.vue` 已整体替换 `ExpressionRuleEditor.vue` 和 `ExpressionRulesPanel.vue`；Reader 的工具栏与正文拖选均进入同一全屏工作台。
- 规则库按惯用语、语法构式、非连续呼应分类，并明确构词/文节由 schema v2 目录管理，表达规则不能回并边界。
- 编辑草稿分别保存语素 mask、文节匹配模式、左右上下文许可、gap、类型、优先级和 annotate-only 输出；上下文开关不再反向改写 mask。
- `ExpressionRulePreview`、核心 `preview_configured_expression_rule` 和 Tauri `preview_expression_rule` 构成无副作用预览链路。预览与保存共用 `compile_user_expression`，返回 accepted/rejected、精确 `matched_ranges`、证据和拒绝原因。
- 工作台每次变化在 120 ms 后请求后端预览；只有本地状态合法且后端 accepted 时才允许保存。规则库先显示再异步加载，IPC 迟缓不会造成按钮激活但界面不出现。
- 旧 `lexical_unit` 不再作为可创建类型；历史条目只在规则库显示为“需迁移”。
